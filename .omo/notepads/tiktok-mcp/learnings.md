...
## T30 MCP Prompts (2026-05-22)

- Three FastMCP prompts ship via `@app.prompt(name=..., description=...)` in `src/tiktok_mcp/prompts/templates.py`: `weekly_marketing_report`, `comment_queue_triage`, `weekly_engagement_summary`. Names are exported as module constants so T31 (and any future test that asserts the prompt count) can import them rather than retyping string literals.
- Quoted tool names in prompt bodies are verbatim from the actual decorated symbols: `marketing_run_async_report`, `marketing_poll_async_report`, `marketing_download_async_report`, `marketing_list_bc_advertisers`, `comments_list`, `display_list_videos`. The Display read tool for user info is `display_get_user_info` (not `display_user_info` as the task spec drafted); we do not reference it from any prompt body so the drift is benign here, but T13's actual exported names are the canonical set.
- FastMCP prompt return type is `list[Message]` where `Message` lives at `mcp.server.fastmcp.prompts.base`. `UserMessage("string")` auto-wraps a string as `TextContent(type="text", text=...)`. Pydantic `validate_call` casts numeric arguments at render time, so `max_comments: int = 50` and `days: int = 7` accept JSON-RPC string args and coerce them.
- `Prompt.arguments` is `list[PromptArgument]` with `name`, `description`, `required` only. Default-valued parameters land as `required=False` automatically; positional-required parameters land as `required=True`. There is no `type` field in `PromptArgument`, so type-shape tests assert via required/optional partitioning.
- Wiring follows the existing tool-import pattern in `src/tiktok_mcp/server.py`: a single side-effect import `from tiktok_mcp.prompts import templates as _prompts_templates` placed alphabetically before the `tools.*` block. Ruff's isort wants the `prompts` import before the `tools` imports because of source-package ordering.
- Subprocess-spawn gotcha: `python -m tiktok_mcp.server` loads `server.py` as `__main__` and triggers a SECOND import of `tiktok_mcp.server` when tool/prompt modules call `from tiktok_mcp.server import app`. The second module gets its own `app` instance, decorators register on it, and `__main__.app.run()` then serves an empty registry. Fix: spawn via `python -c "from tiktok_mcp.server import main; main()"` so the import resolves to the canonical `tiktok_mcp.server` module on the first pass. Future subprocess MCP tests should reuse this pattern.
- No `pytest-timeout` plugin is installed. Integration tests that need a read deadline should use `selectors.DefaultSelector` over `proc.stdout` plus `time.monotonic()` to enforce a per-read timeout rather than the `@pytest.mark.timeout` marker.
- Safety-rail wording in each prompt: `weekly_marketing_report` -> "Safety reminder: this is a read-only flow. No writes required."; `comment_queue_triage` -> names `TIKTOK_MCP_ALLOW_WRITES=comments` explicitly and lists the six gated write tools (`post_comment_reply`, `hide_comment`, `pin_top_comment`, `unpin_top_comment`, `unhide_comment`, `delete_own_reply`) without instructing Claude to call any of them; `weekly_engagement_summary` -> "Safety reminder: this is a read-only summary. No write env vars needed."
- PII discipline: the triage prompt explicitly tells Claude to report only counts, comment_id lists, and proposed reply drafts, never raw `comment_text`. Per DoR §13 this matches the no-cache, no-disk policy for comment bodies that prior waves enforced.
- Tests live at `tests/unit/test_prompts.py` (11 cases covering registration, argument shape, rendered substitutions, safety-language assertions, and a no-credentials sweep) and `tests/integration/test_prompts_e2e.py` (3 cases driving JSON-RPC `prompts/list` plus `prompts/get` against a subprocess). Both files import the prompt name constants instead of repeating string literals.

## T31 stdio entry point (2026-05-22)

- Supported launch form for subprocess MCP tests is `python -m tiktok_mcp`, which resolves through `src/tiktok_mcp/__main__.py` and avoids the `python -m tiktok_mcp.server` double-import trap from T30. The console script remains `tiktok-mcp = "tiktok_mcp.server:main"`.
- FastMCP 1.x stdio in this environment uses newline-delimited JSON, not LSP-style `Content-Length` framing. Source inspection of `mcp.server.stdio.stdio_server` shows it iterates `async for line in stdin` and writes each JSON-RPC message as `model_dump_json(...) + "\n"`.
- Manual boot probe over newline JSON observed first initialize response in 415.7ms and returned a newline JSON frame beginning with `{"jsonrpc":"2.0","id":1,"result":...}`. The T31 test warns only above 1500ms and did not warn in local runs.
