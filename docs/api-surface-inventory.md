...
8. **`posting_get_post_status` vs `get_publish_status` deduplication**: T18 ships `posting_get_post_status` and T27 ships `get_publish_status`. Both call `/v2/post/publish/status/fetch/`. `get_publish_status` is retained as a read-only cached-alias convenience for publish IDs returned by recent upload tools; prefer the alias-aware `posting_get_post_status` for older or multi-account status checks.
